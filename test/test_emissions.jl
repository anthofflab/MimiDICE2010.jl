...
@testset "emissions" begin
    ...
    update_param!(m, :emissions, :YGROSS, read_params(f, "B92:BI92", T))

    # Load the rest of the external parameters
    p = dice2010_excel_parameters(joinpath(@__DIR__, "..", "data", "DICE2010_082710d.xlsx"))
    update_param!(m, :emissions, :sigma, p[:unshared][(:emissions, :sigma)])
    update_param!(m, :emissions, :MIU, p[:shared][:MIU]) # shared parameter
    update_param!(m, :emissions, :etree, p[:unshared][(:emissions, :etree)])
